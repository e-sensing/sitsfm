...
bdc_cerrado_reg <- sits_cube(
    source = "BDC",
    collection = "LANDSAT-OLI-16D",
    data_dir = "/Volumes/KINGSTON/sitsfm/cerrado_cube_reg/"
)
samples_ssl_data <- sits_get_data(
    cube = bdc_cerrado_reg,
    samples = samples_ssl,
    multicores = 8
)

saveRDS(samples_ssl_data, "~/sitsfm/inst/extdata/cerrado_samples/samples_ssl_data_large.rds")

mae_model <- sits_pre_train(
    samples = samples_ssl_data,
